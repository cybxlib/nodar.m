...
направлениях, то вы действительно могли бы определить систему, подобную этой: # $\dfrac{dV}{dl} = F_v(l)u_v(l)$ (dV /dl = Fv(l)u^v(l)), $\dfrac{dN}{dl} = F_n(l)u_n(l)$ (dN /dl = Fn(l)u^n(l)), $\dfrac{dB}{dl} = F_b(l)u_b(l)$ (dB /dl = Fb(l)u^b(l)) - скалярные функции, 
